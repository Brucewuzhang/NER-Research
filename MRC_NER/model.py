from transformers import TFBertModel, BertConfig, TFRobertaModel, RobertaConfig, TFAlbertModel, AlbertConfig
...
class MRCNER(tf.keras.Model):
    ...
    def __init__(self, dropout_rate=0.1, match_dropout_rate=0.3, initializer_range=0.02,
        ...
        if 'roberta' in bert_version:
            ...
            self.bert_config = RobertaConfig.from_pretrained(bert_version)
        elif 'albert' in bert_version:
            self.bert = TFAlbertModel.from_pretrained(bert_version)
            self.bert_config = AlbertConfig.from_pretrained(bert_version)
        else:
...
class MRCBiLSTMNER(tf.keras.Model):
    """
    paper: https://aclanthology.org/2020.acl-main.519.pdf
    """
    ...
    def __init__(self, dropout_rate=0.1, match_dropout_rate=0.3, initializer_range=0.02,
                 bert_version='bert-base-uncased', start_loss_weight=1.0, end_loss_weight=1.0, match_loss_weight=0.1):
        super(MRCBiLSTMNER, self).__init__()
        ...
        # todo: concatenate bilstm start and end output as span context
        # match classification layer
        self.bilstm = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(units=self.lstm_units, return_sequences=True))
        self.match_out = tf.keras.layers.Dense(1, kernel_initializer=tf.keras.initializers.TruncatedNormal(
            stddev=initializer_range))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        bilstm_out = self.bilstm(drop_out)
        ...
        # row_expand = tf.expand_dims(drop_out, axis=2) # [batch_size, seq_len, 1, hidden_size]
        # row_expand = tf.tile(row_expand, [1, 1, seq_len, 1]) # [batch_size, seq_len, seq_len, hidden_size]
        row_expand = self.expand(bilstm_out, axis=2, seq_len=seq_len, rank=4)

        # column_expand = tf.expand_dims(drop_out, axis=1) # [batch_size, 1, seq_len, hidden_size]
        # column_expand = tf.tile(column_expand, [1, seq_len, 1, 1]) # [batch_size, seq_len, seq_len, hidden_size]
        column_expand = self.expand(bilstm_out, axis=1, seq_len=seq_len, rank=4)
